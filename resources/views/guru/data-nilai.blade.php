    ...
    <div class="flex flex-col md:flex-row">
            ...
            <div id="sidebar" class="w-full md:w-64 bg-white min-h-screen transition-all duration-300 overflow-hidden fixed md:relative z-40">
                ...
                <div class="px-4">
                    ...
                    <nav class="space-y-1">
                      ...
                      <a href="{{url ('guru/schedule')}}" class="flex items-center px-4 py-2 text-gray-700 hover:bg-[#49BBBD] hover:text-white rounded-md">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                        </svg>
                        Jadwal
                      </a>
                      <a href="{{url ('guru/data-nilai')}}" class="flex items-center px-4 py-2 text-white bg-[#49BBBD] hover:text-gray-700 rounded-md">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                        </svg>
                        Penilaian
        ...
        <div class="flex-1 p-4 md:p-6">
            ...
            <div class="flex justify-between items-center mb-6">
                <div class="text-2xl font-semibold">Data Nilai</div>
                <div class="flex items-center">
                    <a href="{{ url('/') }}" class="mr-2 text-sm text-gray-700 hover:text-[#49BBBD]">Home</a>
                    <span class="text-gray-400 mx-2">/</span>
                    <div class="text-sm text-gray-500">Data Nilai</div>
                </div>
            </div>

            <!-- Form Filter -->
            <form method="GET" action="{{ route('guru.data-nilai') }}">
                @csrf
                <div class="bg-white p-4 rounded-lg shadow-sm mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Mata Pelajaran</label>
                            <select name="subject_id" id="subject_id" class="w-full p-2 border border-gray-300 rounded-md focus:ring-[#49BBBD] focus:border-[#49BBBD]" onchange="this.form.submit()">
                        ...
                        <div>
                            ...
                            <select id="classSelect" name="class_model_id" class="w-full p-2 border border-gray-300 rounded-md focus:ring-[#49BBBD] focus:border-[#49BBBD]" onchange="this.form.submit()">
                                ...
                                @if($classes && $classes->isNotEmpty())
                                    ...
                                    @endforeach
                                @else
                                    <option disabled>Tidak ada kelas yang tersedia</option>
                                @endif
                            </select>
                        </div>
                        <div>
                            <label for="periodSelect" class="block text-sm font-medium text-gray-700">Pilih Periode</label>
                            <select id="periodSelect" name="period_id" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-[#49BBBD] focus:border-[#49BBBD]" onchange="this.form.submit()">
                        ...
                        <div>
                            <label for="taskSelect" class="block text-sm font-medium text-gray-700">Pilih Tugas</label>
                            <select id="taskSelect" name="task_id" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-[#49BBBD] focus:border-[#49BBBD]" onchange="this.form.submit()">
            ...
            <div class="bg-white rounded-lg shadow-sm overflow-hidden x-auto">
                <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow-md">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-4 py-3 text-left text-sm font-semibold text-gray-600">Nama</th>
                            <th class="px-4 py-3 text-left text-sm font-semibold text-gray-600">Mata Pelajaran</th>
                            <th class="px-4 py-3 text-left text-sm font-semibold text-gray-600">Kelas</th>
                            <th class="px-4 py-3 text-left text-sm font-semibold text-gray-600">Tugas</th>
                            <th class="px-4 py-3 text-left text-sm font-semibold text-gray-600">Nilai</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @if($submissions->isEmpty())
                            <tr>
                                <td colspan="5" class="px-4 py-3 text-sm text-gray-900 text-center">Tidak ada data nilai yang tersedia.</td>
                            </tr>
                        @else
                            @foreach($submissions as $submission)
                                <tr>
                                    <td class="px-4 py-3 text-sm text-gray-900">{{ $submission->user->name }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-900">{{ $submission->task->subject->name }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-900">{{ $submission->user->classModel->class }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-900">{{ $submission->task->title }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-900">{{ $submission->grade }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-900">
                                        {{ \Carbon\Carbon::parse($submission->task->period->start_date)->format('d/m/Y') }} - 
                                        {{ \Carbon\Carbon::parse($submission->task->period->end_date)->format('d/m/Y') }}
                                    </td>
                                </tr>
                            @endforeach
                        @endif
