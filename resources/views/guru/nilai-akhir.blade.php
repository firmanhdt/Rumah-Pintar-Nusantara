    ...
    <div class="flex flex-col md:flex-row">
            ...
            <div id="sidebar" class="w-full md:w-64 bg-white min-h-screen transition-all duration-300 overflow-hidden fixed md:relative z-40">
                ...
                <div class="px-4">
                    ...
                    <nav class="space-y-1">
                      ...
                      <a href="{{url ('guru/nilai-akhir')}}" class="flex items-center px-4 py-2 text-white bg-[#49BBBD] hover:text-gray-700 rounded-md">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                        </svg>
                        Nilai Akhir
        ...
        <div class="flex-1 p-4 md:p-6">
            ...
            <div class="flex justify-between items-center mb-6">
                <div class="text-2xl font-semibold">Nilai Akhir</div>
            ...
            <!-- Form Filter -->
            <form method="GET" action="{{ route('guru.nilai-akhir') }}">
                @csrf
                <div class="bg-white p-4 rounded-lg shadow-sm mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select id="periodSelect" name="period_id" class="w-full p-2 border border-gray-300 rounded-md focus:ring-[#49BBBD] focus:border-[#49BBBD]" onchange="this.form.submit()">
            ...
            <div class="bg-white rounded-lg shadow-sm overflow-hidden x-auto">
                <form method="POST" action="{{ route('guru.store-nilai') }}">
                    ...
                    <input type="hidden" name="class_model_id" value="{{ $request->class_model_id }}">
                    <input type="hidden" name="period_id" value="{{ $request->period_id }}"> <!-- Tambahkan input untuk period_id -->
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach($students as $student)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-900">{{ $subjects->where('id', $request->subject_id)->first()->name ?? 'N/A' }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-900">{{ $student->classModel ? $student->classModel->class : 'N/A' }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-900">
                                        @if(isset($finalGrades[$student->id]))
                                            <input type="number" name="grades[{{ $student->id }}]" value="{{ $finalGrades[$student->id]->final_grade }}" class="border border-gray-300 rounded-md p-1" readonly />
                                        @else
                                            <input type="number" name="grades[{{ $student->id }}]" class="border border-gray-300 rounded-md p-1" placeholder="Masukkan nilai" />
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
                    ...
                    <div class="mt-6 p-4">
                        <button type="submit" class="bg-[#49BBBD] text-white px-6 py-2 rounded-md">Simpan Nilai</button>
                    </div>
                </form>
            </div>
        </div>
